...
ff_stringList = ["Warrior", "Thief", "Monk", "White Mage", "Black Mage", "Red Mage"]
...
def addfourPartyMems(num, ff_party):
...
def makePartyHumanReadable(ff_party, ff_party_string_list, ff_party_list):
    for ff_class in ff_party_list:
        for i in range(4):
            if ff_class == ff_party[i]:
                print(ff_party_string_list[i])
...
while len(ff_party) > 4:
    if len(ff_party) == 5:
        addfourPartyMems(4, ff_party)
    elif len(ff_party) == 6:
        addfourPartyMems(5, ff_party)

print("Final: ")
makePartyHumanReadable(ff_party, ff_party_string_list=ff_stringList, ff_party_list=ff_classes)